...
<?php
    ...
    $sql_select = "select * from qnaboard where num = $num";
    ...
    if ($userid !== $id) {
        $new_hit = $hit + 1;
        $sql_update = "update qnaboard set hit = $new_hit where num = $num";
...
?>
  ...
  <div id="reply">
    ...
    <div id="reply3">
<?php
    $sql_select = "select * from `qnaboard_ripple` where parent = '$num'";
...
<?php
            if($_SESSION['id'] == "admin" || $_SESSION['id'] ==  $reply_id){
              echo "
                ...
                action='./qnaboard_server.php' method='post'>
...
<?php
          ...
          mysqli_close($con);
?>
      <form action="http://<?= $_SERVER['HTTP_HOST']; ?>/ALREADY_AWESOME/board/qnaboard_server.php" method = "post" name ="repple_form">
